"""
Format the monthly signal report for email delivery.
"""
...
def format_report(
    ...
    """
    Format the complete email report.
    
    Returns:
        (subject, body) tuple of strings.
    """
    subject = f"[TAA] Tranche {tranche} Signals — {signal_date}"
    
    lines = []
    lines.append("=" * 60)
    lines.append(f"  TAA Monthly Signal Report — Tranche {tranche} of 2")
    ...
    # Sort: largest positions first, cash/BIL last
    sorted_alloc = sorted(
